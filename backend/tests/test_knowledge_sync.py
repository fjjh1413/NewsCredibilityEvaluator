...
class KnowledgeSyncTestCase(unittest.TestCase):
    ...
    @patch("app.services.knowledge_service.enqueue_knowledge_index_job")
    @patch("app.services.knowledge_service.knowledge_crud.create_knowledge_item")
    def test_create_queues_index_job_without_calling_chroma(
        self,
        mocked_create,
        mocked_enqueue,
    ) -> None:
        ...
        result = create_knowledge_item(db, _payload())

        self.assertEqual(result.vector_sync_status, "pending")
        mocked_create.assert_called_once()
        self.assertFalse(mocked_create.call_args.kwargs["auto_commit"])
    ...
    def test_update_commits_after_index_job_is_queued(
        self,
        mocked_get,
        mocked_update,
        mocked_enqueue,
    ...
    @patch("app.services.knowledge_service.knowledge_crud.update_knowledge_item")
    @patch("app.services.knowledge_service.get_knowledge_item")
    def test_update_rolls_back_when_index_job_enqueue_fails(
        self,
        mocked_get,
        mocked_update,
        mocked_enqueue,
        ...
        mocked_get.return_value = item
        mocked_enqueue.side_effect = RuntimeError("queue failed")
    ...
    def test_update_persists_mysql_content_and_queues_index_job(self) -> None:
        engine = create_engine("sqlite:///:memory:")
        KnowledgeItem.__table__.create(bind=engine)
        KnowledgeIndexJob.__table__.create(bind=engine)
        ...
        queued_job = verify_db.query(KnowledgeIndexJob).first()

        self.assertIsNotNone(persisted_item)
        self.assertEqual(persisted_item.title, "New title")
        self.assertEqual(persisted_item.content, "New content")
        self.assertEqual(persisted_item.vector_sync_status, "pending")
        self.assertIsNone(persisted_item.vector_sync_error)
        self.assertIsNotNone(queued_job)
        self.assertEqual(queued_job.knowledge_id, 1)
